...
def spider_run(parse_config_value, device_ip, iter_status=False, rec=False):
    try:
        ...
        we_run_app.check_spider_status(iter_status=iter_status)
        ...
        spider_run(parse_config_value, device_ip, iter_status=iter_status)
    except RecursionError as e:
        print(e)
        spider_run(parse_config_value, device_ip, iter_status=iter_status, rec=True)


@app.task()
def main(device, iter_status):
    parse_config_value = WeConfigParse.parse_base_config('config/config.yaml')
    while True:
        spider_run(parse_config_value, device, iter_status)
